...
fn render_candlestick_series(
    ...
    let mut horizontal_offset: f64 = 0.0; // Initialize horizontal offset
    let scroll_speed: f64 = 20.0; // Speed of panning, adjust as needed

    while let Some(event) = window.next() {
        // Handle mouse scroll events to update horizontal_offset
        ...
        window.draw_2d(&event, |context, graphics, _| {
            ...
            // Draw OHLC data with horizontal offset
            for (i, ohlc) in scaled_data.iter().enumerate() {
                // Adjust x positions by horizontal offset
